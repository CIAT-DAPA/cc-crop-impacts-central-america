...
library(maptools)
library(stringr)
source("E:/_scripts/cc-crop-impacts-central-america/src-ecocrop/createMask.R")
source("E:/_scripts/cc-crop-impacts-central-america/src-ecocrop/impacts.R")
...
shpSubLevel <- "E:/Tortillas/TORII/EcoCrop_runs/00_admin_data/CAMEXCA_adm0_diss.shp"
...
for (i in 1:length(cropLs)){
  ...
  mFile <- read.csv(paste0(iDir, "/impacts-", cropLs[i], "/impacts-", reg, ".csv"))
...
write.csv(crop_stat, paste(iDir, "/impacts-", reg, "-allcrops-whole-area.csv", sep=""), row.names=F, quote=T)


# Make the boxplots for three variables: 
crop_stat <- read.csv(paste(iDir, "/impacts-", reg, "-allcrops-whole-area.csv", sep=""), header = T)
# crop_stat <- crop_stat[which(crop_stat$SSP == "SSP45"), ]

##### Overall suitability changev #####

if (!file.exists(paste0(oDir, "/osc_all_crops_whole_area.tif"))) {
  
  tiff(paste0(oDir, "/osc_all_crops_whole_area.tif"), width=4200, height=1000, pointsize=20, compression='lzw',res=300)
...
if (!file.exists(paste0(oDir, "/pia_nia_ratio_all_crops_whole_area.tif"))) {
  
  tiff(paste0(oDir, "/pia_nia_ratio_all_crops_whole_area.tif"), width=4200, height=1000, pointsize=20, compression='lzw',res=300)
...
if (!file.exists(paste0(oDir, "/chg_area_all_crops_whole_area.tif"))) {
  
  tiff(paste0(oDir, "/chg_area_all_crops_whole_area.tif"), width=4200, height=1000, units = "px", pointsize=20, compression='lzw',res=300)
