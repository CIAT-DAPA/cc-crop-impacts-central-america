...
for (ssp in sspList) {
  ...
  for (gcm in gcmList){
    ...
    for (var in varList){
      ...
      oPlot <- paste(oDir, "/plot_season_anom_", var, "_", ssp, "_", gcm, ".tif", sep="")
      
      if (!file.exists(oPlot)) {
        
        stk <- stack(paste0(baseDir, "/", ssp, "/", gcm, "/", perSeas[,2], "/", var, "_", perSeas[,1], ".tif"))
        ...
        tiff(paste(oDir, "/plot_seasonal_anom_", var, "_", ssp, "_", gcm, ".tif", sep=""), width=3600, height=1200, compression='lzw',res=200)
