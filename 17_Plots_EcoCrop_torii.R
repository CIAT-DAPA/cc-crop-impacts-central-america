...
oDir <- "E:/Tortillas/TORII/EcoCrop_runs/03_crop_impacts/evaluation/suit_change_torii"
uDir <- "E:/Tortillas/TORII/EcoCrop_runs/03_crop_impacts/uncertainties"
mask <- readOGR("E:/Tortillas/TORII/EcoCrop_runs/00_admin_data/TORII_adm0.shp")

sspLs <- c("ssp_126", "ssp_585")
sspLsMod <- c("SSP1-2.6", "SSP5-8.5")
yearLs <- c("2030s", "2050s")
id <- c("2030s SSP1-2.6", "2050s SSP1-2.6", 
        "2030s SSP5-8.5", "2030s SSP5-8.5"
)
...
for(crop in cropLs){
  ...
  if(!file.exists(paste(oDir, "/suitchg_", crop, ".tif", sep=""))){
    ...
    print(levelplot(plot, at = zvalues,  
                    ...
                    names.attr=c(yearLs, rep("", length(id)-2)),
                    layout=c(2, 2), 
                    main="",
                    xlab="",
